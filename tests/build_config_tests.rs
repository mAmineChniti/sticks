...
#[test]
fn test_build_config_apply_to_makefile_no_cxxflags() {
	let temp_dir = TempDir::new().unwrap();
	let makefile_path = temp_dir.path().join("Makefile");
...
"#;
	...
	let result = fs::read_to_string(&makefile_path).unwrap();
	assert!(result.contains("CXXFLAGS"));
	assert!(result.contains("-std=c++17"));
}

#[test]
fn test_compiler_flags_remove_nonexistent() {
	let mut flags = CompilerFlags::new();
	flags.add_flag("-Wall");
	flags.remove_flag("-Wextra");
	assert_eq!(flags.flags.len(), 1);
}

#[test]
fn test_preprocessor_defs_remove_nonexistent() {
	let mut defs = PreprocessorDefs::new();
	defs.add_def("DEBUG");
	defs.remove_def("NDEBUG");
	assert_eq!(defs.defs.len(), 1);
}

#[test]
fn test_include_dirs_remove_nonexistent() {
	let mut dirs = IncludeDirs::new();
	dirs.add_dir("include");
	dirs.remove_dir("src");
	assert_eq!(dirs.dirs.len(), 1);
...
#[test]
fn test_library_dirs_remove_nonexistent() {
	let mut dirs = LibraryDirs::new();
	dirs.add_dir("/usr/local/lib");
	dirs.remove_dir("lib");
	assert_eq!(dirs.dirs.len(), 1);
...
#[test]
fn test_cpp_standard_cpp23() {
	assert_eq!(CppStandard::from_str("23").unwrap(), CppStandard::Cpp23);
	assert_eq!(CppStandard::Cpp23.to_cmake_flag(), "23");
	assert_eq!(CppStandard::Cpp23.to_gcc_flag(), "-std=c++23");
}
