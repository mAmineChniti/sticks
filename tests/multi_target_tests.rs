...
#[test]
fn test_multi_target_manager_add_shared_library_to_cmake() {
	let temp_dir = TempDir::new().unwrap();
	let cmake_path = temp_dir.path().join("CMakeLists.txt");
...
"#;
	...
	let mut manager = MultiTargetManager::new();
	let mut lib_target = BuildTarget::new("mylib".to_string(), TargetType::SharedLibrary);
	lib_target.add_source("src/lib.cpp".to_string());
	lib_target.add_dependency("pthread".to_string());
	manager.add_target(lib_target);

	manager.add_to_cmake(&cmake_path).unwrap();

	let result = fs::read_to_string(&cmake_path).unwrap();
	assert!(result.contains("add_library(mylib SHARED"));
	assert!(result.contains("src/lib.cpp"));
	assert!(result.contains("target_link_libraries(mylib"));
}

#[test]
fn test_multi_target_manager_add_shared_library_to_makefile() {
	let temp_dir = TempDir::new().unwrap();
	let makefile_path = temp_dir.path().join("Makefile");
...
"#;
	...
	let result = fs::read_to_string(&makefile_path).unwrap();
	assert!(result.contains("libmylib.so"));
	assert!(result.contains("src/lib.cpp"));
}

#[test]
fn test_multi_target_manager_add_static_library_to_makefile() {
	let temp_dir = TempDir::new().unwrap();
	let makefile_path = temp_dir.path().join("Makefile");
...
"#;
	...
	let mut manager = MultiTargetManager::new();
	let mut lib_target = BuildTarget::new("mylib".to_string(), TargetType::StaticLibrary);
	lib_target.add_source("src/lib.cpp".to_string());
	manager.add_target(lib_target);
	...
	let result = fs::read_to_string(&makefile_path).unwrap();
	assert!(result.contains("libmylib.a"));
	assert!(result.contains("src/lib.cpp"));
}

#[test]
fn test_multi_target_manager_duplicate_target_prevention() {
	let mut manager = MultiTargetManager::new();
	let target1 = BuildTarget::new("main".to_string(), TargetType::Executable);
	let target2 = BuildTarget::new("main".to_string(), TargetType::Executable);
...
#[test]
fn test_build_target_source_deduplication() {
	let mut target = BuildTarget::new("main".to_string(), TargetType::Executable);
	target.add_source("src/main.cpp".to_string());
	target.add_source("src/main.cpp".to_string());
...
#[test]
fn test_build_target_dependency_deduplication() {
	let mut target = BuildTarget::new("main".to_string(), TargetType::Executable);
	target.add_dependency("pthread".to_string());
	target.add_dependency("pthread".to_string());
...
#[test]
fn test_multi_target_manager_get_target() {
	let mut manager = MultiTargetManager::new();
	let target = BuildTarget::new("main".to_string(), TargetType::Executable);
	manager.add_target(target.clone());

	let retrieved = manager.get_target("main");
	assert!(retrieved.is_some());
	assert_eq!(retrieved.unwrap().name, "main");
}

#[test]
fn test_multi_target_manager_get_nonexistent_target() {
	let manager = MultiTargetManager::new();
	let retrieved = manager.get_target("nonexistent");
