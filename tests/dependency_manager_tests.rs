...
#[test]
#[serial]
fn test_dependency_manager_add_multiple_makefile() {
	let temp_dir = env::temp_dir().join(format!(
		"sticks_test_dm_multi_{}",
		std::time::SystemTime::now()
			.duration_since(std::time::UNIX_EPOCH)
			.unwrap()
			.as_nanos()
	...
	let deps = match pm {
		...
		_ => vec!["curl".to_string(), "openssl".to_string()],
	};
...
#[test]
#[serial]
fn test_dependency_manager_remove_cmake() {
	let temp_dir = env::temp_dir().join(format!(
		"sticks_test_dm_remove_cmake_{}",
		std::time::SystemTime::now()
			.duration_since(std::time::UNIX_EPOCH)
			.unwrap()
			.as_nanos()
	...
	let result = DependencyManager::remove(&["CURL".to_string()]);
	assert!(result.is_ok());
...
#[test]
#[serial]
fn test_dependency_manager_list_empty_makefile() {
	let temp_dir = env::temp_dir().join(format!(
		"sticks_test_dm_empty_{}",
		std::time::SystemTime::now()
			.duration_since(std::time::UNIX_EPOCH)
			.unwrap()
			.as_nanos()
...
#[test]
#[serial]
fn test_dependency_manager_list_empty_cmake() {
	let temp_dir = env::temp_dir().join(format!(
		"sticks_test_dm_empty_cmake_{}",
		std::time::SystemTime::now()
			.duration_since(std::time::UNIX_EPOCH)
			.unwrap()
			.as_nanos()
