	...
	if found_dependencies {
			...
			&& !temp_updated_makefile_content.contains("sudo apt install -y")
		{
			...
			if updated_makefile_content.contains("all: clean install-deps") {
				updated_makefile_content = updated_makefile_content.replace("all: clean install-deps", "all: clean");
			}
...
fn add_sources(source_names: &[&str]) -> io::Result<()> {
	if !Path::new("src").exists() {
        print_colored("src directory not found. Cannot add sources and headers.","31",1);
...
fn create_project(project_name: &str, language: Language) -> io::Result<()> {
	...
	let makefile_content = format!(
		...
		clean:\n\
		\trm -f {}\n",
		cc, project_name, project_name, language_extension(&language), project_name, project_name
	);
...
fn main() {
	...
	match matches.subcommand() {
		...
		("src", Some(sub_m)) => {
			// Implement the removal logic when needed
			let sources: Vec<&str> = sub_m.values_of("source_names").unwrap_or_default().collect();
